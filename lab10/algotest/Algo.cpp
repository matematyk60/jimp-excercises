...
namespace algo{
    ...
    vector<int> Values(const map<string, int> &m) {
        vector <int> answer;
        std::transform(m.begin(), m.end(), back_inserter(answer), [](pair<string,int> tmp){ return tmp.second;});
        return answer;
    }

    map<string, int> DivisableBy(const map<string, int> &m, int divisor) {
        map <string, int> answer;
        copy_if(m.begin(), m.end(), inserter(answer,answer.end()), [divisor](pair<string,int> tmp){return (tmp.second % divisor) == 0;});
        return answer;
    }
    ...
    bool ContainsValue(const map<string, int> &v, int value) {
        return any_of(v.begin(), v.end(), [value](pair<string, int> tmp) { return tmp.second == value;});
    }
