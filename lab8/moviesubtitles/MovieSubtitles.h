...
namespace moviesubs {
    ...
    public:
        InvalidSubtitleLineFormat(std::string message = "Invalid subtitle line format", int line = 0)
                : SubShiftingError("At line " + std::to_string(line) + ": " + message, line) {}
    };

    class NegativeFrameAfterShift : public SubShiftingError {
    public:
        NegativeFrameAfterShift(std::string message, int line) : SubShiftingError(
                "At line " + std::to_string(line) + ": " + message, line) {}
    };

    class SubtitleEndBeforeStart : public SubShiftingError {
    public:
        SubtitleEndBeforeStart(std::string message, int line) : SubShiftingError(
                "At line " + std::to_string(line) + ": " + message, line) {}
    };

    class MissingTimeSpecification : public SubShiftingError {
    public:
        MissingTimeSpecification(std::string message, int line) : SubShiftingError(
                "At line " + std::to_string(line) + ": " + message, line) {}
    };
